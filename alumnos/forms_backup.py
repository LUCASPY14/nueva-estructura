...
class MisTransaccionesFilterForm(forms.Form):
    """
    Formulario para filtrar transacciones en la vista de padres
    """
    alumno = forms.ModelChoiceField(
        queryset=None,  # Se configura dinámicamente
        required=False,
        empty_label="Todos mis hijos",
        widget=forms.Select(attrs={
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and hasattr(user, 'padre_profile'):
            ...
            # Fallback a todos los alumnos activos
            self.fields['alumno'].queryset = Alumno.objects.filter(estado='activo')

class SolicitudCargaSaldoForm(forms.ModelForm):
    """
    Formulario mejorado para solicitar carga de saldo
    """
    class Meta:
        model = SolicitudRecarga
        fields = ['alumno', 'monto_solicitado', 'metodo_pago', 'referencia_pago', 'comprobante_pago']
        ...
        help_texts = {
            'monto_solicitado': 'Monto mínimo: 1.000 Gs., máximo: 500.000 Gs.',
            'comprobante_pago': 'Formatos: JPG, PNG, PDF (máx. 5MB)',
            'referencia_pago': 'Número de transacción o comprobante del banco/billetera'
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if user and hasattr(user, 'padre_profile'):
            # Solo alumnos del padre logueado
            self.fields['alumno'].queryset = user.padre_profile.alumnos.filter(estado='activo')
        else:
            self.fields['alumno'].queryset = Alumno.objects.filter(estado='activo')
    ...
    def clean_monto_solicitado(self):
        ...
        if monto:
            if monto < 1000:
                raise ValidationError('El monto mínimo es de 1.000 Gs.')
            if monto > 500000:
                raise ValidationError('El monto máximo es de 500.000 Gs.')
        return monto
...
class HistorialTransaccionesFilterForm(forms.Form):
    ...
    tipo_transaccion = forms.ChoiceField(
        choices=[
            ('', 'Todos los tipos'),
            ('recarga', 'Solo Recargas'),
            ('consumo', 'Solo Consumos'),
            ('ajuste', 'Solo Ajustes'),
            ('devolucion', 'Solo Devoluciones')
        ],
        required=False,
        widget=forms.Select(attrs={
            'class': 'border rounded px-3 py-2 bg-white'
...
class EliminarAlumnoForm(forms.Form):
    ...
    confirmar = forms.BooleanField(
        required=True,
        widget=forms.CheckboxInput(attrs={
            'class': 'mr-2 h-4 w-4 text-red-600 focus:ring-red-500'
        }),
        label='Confirmo que deseo eliminar este alumno y todas sus transacciones asociadas'
    )
    ...
    motivo = forms.CharField(
        ...
        widget=forms.Textarea(attrs={
            ...
            'rows': 3,
            'placeholder': 'Motivo de la eliminación (opcional pero recomendado para auditoría)'
        }),
...
class DashboardAdminFilterForm(forms.Form):
    ...
    estado_alumno = forms.ChoiceField(
        choices=[
            ('', 'Todos los estados'),
            ('activo', 'Solo activos'),
...
class FiltroRapidoTransaccionesForm(forms.Form):
    """
    Formulario simple para filtros rápidos de transacciones
    """
    PERIODO_CHOICES = [
        ('hoy', 'Hoy'),
        ('ayer', 'Ayer'),
